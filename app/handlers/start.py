...
async def show_programs_menu(message: Message, state: FSMContext):
    """Показать подменю 'Мои Программы тренировок' (устаревшая кнопка, оставлена для совместимости)."""
    # Эта кнопка больше не используется, так как все функции теперь в главном меню
    await message.answer(
        "Все функции теперь доступны в главном меню.",
        reply_markup=get_main_keyboard()
    )
